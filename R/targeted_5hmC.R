...
save(bed.list, file="manuscript/bed.list.5hmC.RData")
...
load("manuscript/bed.list.5hmC.RData")
names(bed.list)
pdata <- read.csv("manuscript/pdata.5mC.csv", row.names = 1)
...
par(mar = c(15,15,8,10), mfrow = c(1,1))
vioplot(block.sizes.df, las = 2)
...
#  scale_color_manual(values = group.colors) +
  ggtitle("PCA integrated data (5hmC)")
...
load("manuscript/BSobj_5hmCpG.RData")
...
for(i in 1:nrow(regions)){
  ...
  jpeg(filename = paste0("5hmC_",rownames(regions)[i],".jpeg"), width = 900, height = 600, quality = 100)
...
dmrs = callDMR(dml.test, p.threshold=0.05, delta = 0) # no delta threshold used for 5hmC
head(dmrs)
...
# Treg vs Th0
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Treg.vs.Th0.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Treg.vs.Th0.csv")
# Th1 vs Th0
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Th1.vs.Th0.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Th1.vs.Th0.csv")
# Th2 vs Th0
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Th2.vs.Th0.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Th2.vs.Th0.csv")
# Th17 vs Th0
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Th17.vs.Th0.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Th17.vs.Th0.csv")
# Th17.no.TGFb vs Th17
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Th17.no.TGFb.vs.Th17.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Th17.no.TGFb.vs.Th17.csv")
# Th17.no.Tgfb vs Th0
write.csv(dmls, file = "manuscript/tables/DMLs_5hmC_Th17.no.TGFb.vs.Th0.csv")
write.csv(dmrs, file = "manuscript/tables/DMRs_5hmC_Th17.no.Tgfb.vs.Th0.csv")
...
targets <- read.csv("manuscript/tables/DhMRs/DMRs_5hmC_Th2.vs.Th0.csv", row.names = 1)
...
#targets.0 <- read.csv("manuscript/tables/DhMRs/DMRs_5hmC_Th2.vs.Th0.csv", row.names = 1)
targets.1 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Treg.vs.Th0.csv", row.names = 1)
targets.1$start <- targets.1$pos
targets.1$end <- targets.1$pos
targets.2 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Th1.vs.Th0.csv", row.names = 1)
targets.2$start <- targets.2$pos
targets.2$end <- targets.2$pos
targets.3 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Th2.vs.Th0.csv", row.names = 1)
targets.3$start <- targets.3$pos
targets.3$end <- targets.3$pos
targets.4 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Th17.vs.Th0.csv", row.names = 1)
targets.4$start <- targets.4$pos
targets.4$end <- targets.4$pos
targets.5 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Th17.no.TGFb.vs.Th0.csv", row.names = 1)
targets.5$start <- targets.5$pos
targets.5$end <- targets.5$pos
targets.6 <- read.csv("manuscript/tables/DhMLs/DMLs_5hmC_Th17.no.TGFb.vs.Th17.csv", row.names = 1)
targets.6$start <- targets.6$pos
targets.6$end <- targets.6$pos
...
pheatmap::pheatmap(heatmap.data,
                   ...
                   angle_col = 45,
                   border_color = "grey",
                   main = "Cell type specific DMLs (5hmC)",
                   fontsize = 14,
                   cellwidth = 20,
                   cellheight = 6)
